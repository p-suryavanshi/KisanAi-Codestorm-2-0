...
class ProfileUpd(BaseModel):
    name: str = None; state: str = None
    crop: str = None; soil: str = None; land: float = None
...
async def api_register(req: RegisterReq):
    ...
    if len(req.mobile) < 10:
        return JSONResponse({"error": "Invalid mobile number"}, status_code=400)
    if len(req.password) < 6:
        return JSONResponse({"error": "Password must be at least 6 characters"}, status_code=400)
    user, err = Auth.register(req.name.strip(), req.mobile, req.password, req.state, req.crop)
    if err:
        return JSONResponse({"error": err}, status_code=400)
    _, token, _ = Auth.login(req.mobile, req.password)
...
async def api_login(req: LoginReq):
    user, token, err = Auth.login(req.mobile, req.password)
    if err:
...
async def api_me(kisanai_token: str = Cookie(default=None)):
    ...
    return {"name": user["name"], "mobile": user["mobile"],
            ...
            "joined": user["joined"], "badges": user.get("badges",[]),
...
# ── Override chat to save history for logged-in users ──────
@app.post("/api/chat/auth")
async def chat_auth(req: ChatRequest, kisanai_token: str = Cookie(default=None)):
    """Chat with history saving for logged-in users"""
    # Use profile from account if logged in
    user = Auth.get_user(kisanai_token)
    if user:
        req.crop = req.crop or user.get("crop", "wheat")
        req.state = req.state or user.get("state", "MP")
        req.soil = req.soil or user.get("soil", "black")
        req.land = req.land or user.get("land", 2.5)
    ...
    if OPENAI_API_KEY:
        ...
        r = DEMO_RESPONSES[lang]
        if any(w in m for w in ["yellow","पीले","colour","color","leaves","पत्ते","pale"]): reply = r["yellow"]
        elif any(w in m for w in ["irrigat","water","सिंचाई","पानी"]): reply = r["irrigat"]
        elif any(w in m for w in ["fertil","urea","खाद","उर्वरक","dap","npk"]): reply = r["fertil"]
        elif any(w in m for w in ["pest","insect","bug","कीट","disease","रोग"]): reply = r["pest"]
        elif any(w in m for w in ["price","market","sell","भाव","मंडी"]): reply = r["market"]
        elif any(w in m for w in ["sow","season","when","बुवाई","मौसम"]): reply = r["season"]
        else: reply = r["default"]
        result = {"reply": reply, "mode": "demo"}
